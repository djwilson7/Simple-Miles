...
// 1) Generic gradient-track slider
    ...
    var body: some View {
        ZStack {
            ...
            Slider(value: $value, in: range)
                .tint(.clear)               // hide the native tint track
                .labelsHidden()
                .padding(.horizontal, -2)   // align knob closer to track edges
        }
        .frame(height: height)
    }
}

// 2) Helpers to make HSB gradients
extension Color {
    static func hsb(h: Double, s: Double, b: Double) -> Color {
        Color(UIColor(hue: CGFloat(h), saturation: CGFloat(s), brightness: CGFloat(b), alpha: 1))
    }
}
